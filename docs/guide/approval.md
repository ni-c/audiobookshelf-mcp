...
| Tool                            | What it takes out                                                             |
| ------------------------------- | ----------------------------------------------------------------------------- |
| `delete_collection`             | the curated list and its order; the books stay                                |
| `delete_playlist`               | the playlist; the media stays                                                 |
| `delete_media_progress`         | position, finished state and dates — the listening history of an item         |
...
- `create_bookmark` makes a new bookmark at that position, with a new title. The
  one somebody typed is gone, and the position is the only thing
  `delete_bookmark` is given — a wrong `time` takes out a different bookmark than
...
That is not a formatting preference. Book descriptions come from metadata
providers, podcast titles from RSS feeds, collection names from whoever uses the
server. None of it is written by you, and the prompt is read by a model at the
exact moment it is deciding whether to delete something.
...
The approval is bound to its target, so one obtained for a collection cannot be
...
`ELICITATION` has no `AUDIOBOOKSHELF_` in front of it, so one
...
`update_collection` is exactly that case: it replaces a name somebody typed, and
