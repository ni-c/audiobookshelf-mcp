...
describe('selecting tools', () => {
  it('narrows tools/list to an allow list', async () => {
    ...
    ).toEqual(['get_author', 'get_collection'].toSorted());
  });
  ...
  it('subtracts the deny list from the allow list', async () => {
    expect(
      await toolNames({
        allowTools: 'get_author,get_collection',
        denyTools: 'get_collection',
      })
    ).toEqual(['get_author']);
  });
  ...
  it('lets the preset compose with extra names', async () => {
    expect(
      await toolNames({ allowTools: 'essential,add_books_to_collection' })
    ).toEqual([...ESSENTIAL_TOOLS, 'add_books_to_collection'].toSorted());
  });
...
describe('a filtered-out tool', () => {
  it('cannot be called either, not merely hidden', async () => {
    ...
    const server = createServer(config({ allowTools: 'get_author' }));
    const client = new Client({ name: 'test-client', version: '0.0.0' });
    const [clientTransport, serverTransport] =
      InMemoryTransport.createLinkedPair();
    await Promise.all([
    ...
    await expect(
      client.callTool({
        name: 'add_books_to_collection',
        arguments: {},
      })
    ).rejects.toThrow('Tool add_books_to_collection not found');
...
describe('refusing an unusable list', () => {
  ...
  it('applies the same rule to the deny list', () => {
    expect(() => createServer(config({ denyTools: 'get_authoz' }))).toThrow(
      /_DENY_TOOLS: no tool matches "get_authoz"/
    );
  });
...
describe('together with read-only mode', () => {
  ...
  it('names read-only as the reason, rather than calling the tool unknown', () => {
    ...
    try {
      createServer(
        config({ ...readOnly, allowTools: 'add_books_to_collection' })
      );
    } catch (error) {
      thrown = error;
    }
  ...
  it('does not apply the write-tool rule to the deny list', async () => {
    ...
    expect(
      await toolNames({ ...readOnly, denyTools: 'add_books_to_collection' })
